...
// Upload all files from integration package
async function uploadAllFiles(token, repoName) {
        ...
        for (const item of items) {
            ...
            if (item.isDirectory()) {
                ...
                try {
                    ...
                    await uploadFileToGitHub(token, repoName, uploadPath, fileContent);
...
async function setupGitHubRepository(jobId, token, repoName, description) {
    ...
    try {
        ...
        await uploadAllFiles(token, uniqueRepoName);
